...
```js
npm install babel-eslint eslint eslint-config-prettier eslint-loader eslint-plugin-vue --save-dev
```
...
module.exports = {
  ...
  parserOptions: {
    parser: 'babel-eslint', //指定解析器 默认esprima  ts:@typescript-eslint/parser
    ecmaVersion: 6, //默认5 想使用es6语法
    sourceType: 'module', //默认script 可选module(ECMAScript模块)
    ecmaFeatures: { //想使用额外语言的特性
      "jsx": true
    }
  },
  // 配置插件 插件名称省略eslint-plugin-，用来配置规范的vue，html等
  plugins: [
    'vue',
    //'react'
    //'html'
  ],
  //使用插件 eslint-plugin-vue插件，vue/strongly-recommended规则
  extends: [
